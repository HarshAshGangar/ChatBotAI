...
import 'package:seventhapp/utils/imagepicker.dart';

class SendImageScreen extends ConsumerStatefulWidget {

  const SendImageScreen({super.key, required String sessionId}); // Make it required
...
class _SendImageScreenState extends ConsumerState<SendImageScreen> {
  ...
  late final String sessionId; // Add sessionId as a parameter
  ...
  void _pickImage() async {
    final pickedImage = await pickImage(); // Calls the external `pickImage` function
    if (pickedImage == null) {
      return;
    }
              ...
              child: isLoading
                ...
                onPressed: () async {
                  ...
                  await ref.read(chatProvider).sendMessage(
                    apiKey: apiKey,
                    image: image,
                    promptText: _promptController.text.trim(), sessionId: sessionId,
                  );
